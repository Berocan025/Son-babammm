...
// Get content with variables replacement
function getContentWithVariables($key, $default = '') {
    global $pdo;
    $content = getContent($key, $default);
    
    // Replace common variables
    $content = str_replace('[SITE_NAME]', getSetting('site_title', 'Portfolio'), $content);
    $content = str_replace('[SITE_BRAND]', getSetting('site_brand', 'BERAT K - R10'), $content);
    $content = str_replace('[CONTACT_EMAIL]', getSetting('contact_email', 'info@example.com'), $content);
...
function getContent($key, $default = '') {
    global $pdo;
    try {
        $stmt = $pdo->prepare("SELECT content_text FROM site_contents WHERE content_key = ?");
        $stmt->execute([$key]);
        $result = $stmt->fetch();
        return $result ? $result['content_text'] : $default;
    } catch(PDOException $e) {
        return $default;
    }
}
...
// Set content
function setContent($key, $value, $title = '', $type = 'text', $location = 'general') {
    global $pdo;
    try {
        // Check if content exists
        $stmt = $pdo->prepare("SELECT id FROM site_contents WHERE content_key = ?");
        $stmt->execute([$key]);
        
        if ($stmt->fetch()) {
            // Update existing
            $stmt = $pdo->prepare("UPDATE site_contents SET content_text = ?, content_title = ?, content_type = ?, page_location = ? WHERE content_key = ?");
            return $stmt->execute([$value, $title, $type, $location, $key]);
        } else {
            // Insert new
            $stmt = $pdo->prepare("INSERT INTO site_contents (content_key, content_title, content_text, content_type, page_location) VALUES (?, ?, ?, ?, ?)");
            return $stmt->execute([$key, $title, $value, $type, $location]);
        }
    } catch(PDOException $e) {
        return false;
    }
...
function truncateText($text, $limit = 100, $readMoreText = 'Devamını Oku') {
    ...
    return $truncated . '... <a href="#" class="read-more-link" onclick="toggleReadMore(this)">' . $readMoreText . '</a>
